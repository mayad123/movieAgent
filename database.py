...
class Database:
    ...
    def save_response(self, request_id: str, response_text: str, sources: List[Dict] = None,
                     token_usage: Dict = None, cost_usd: float = None):
        """Save response data."""
        try:
            cursor = self.conn.cursor()
    ...
    def save_metric(self, request_id: str, metric_type: str, metric_name: str,
                   metric_value: float = None, metric_data: Dict = None):
        """Save a metric."""
        try:
            cursor = self.conn.cursor()
    ...
    def get_stats(self, days: int = 7, request_type: Optional[str] = None, outcome: Optional[str] = None) -> Dict:
