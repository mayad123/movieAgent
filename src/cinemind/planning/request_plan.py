...
class RequestPlan:
    ...
    freshness_reason: str | None = None  # Reason for final freshness decision
...
class RequestPlanner:
    ...
    async def plan_request(self, prompt: str, client: Any, request_type: str | None = None) -> RequestPlan:
        """
        ...
        if not structured_intent.requires_disambiguation and structured_intent.candidate_year is not None:
            logger.warning("Schema validation: candidate_year set to None (requires_disambiguation is False)")
    ...
    def _select_tools(self, intent: str, request_type: str) -> list[ToolType]:
        ...
        if request_type == "info" or request_type == "release-date":
    ...
    def _extract_entity_years(self, prompt: str, entities: list[str]) -> dict[str, int | None]:
        ...
        re.findall(year_pattern, prompt)
