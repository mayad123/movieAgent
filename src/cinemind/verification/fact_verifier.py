...
class FactVerifier:
    ...
    def extract_release_year(self, title: str, sources: list) -> VerifiedFact | None:
        """
        ...
        conflicts = []
        if len(years_found) > 1:
            other_years = [y for y in years_found if y != most_common_year]
    ...
    def resolve_conflicts(self, facts: list[VerifiedFact]) -> list[VerifiedFact]:
        ...
        # Resolve each group
        for _key, group in fact_groups.items():
            if len(group) == 1:
