...
from ..infrastructure.tagging import HybridClassifier, RequestTagger
from ..llm.client import HttpChatLLMClient, LLMClient
from ..media.media_enrichment import attach_media_to_result
from ..planning.request_plan import RequestPlan, RequestPlanner
from ..planning.source_policy import SourcePolicy
from ..planning.tool_plan import ToolPlanner
from ..prompting import EvidenceBundle, PromptBuilder, get_template
from ..prompting.output_validator import OutputValidator
from ..search.search_engine import MovieDataAggregator, SearchEngine
...
class CineMind:
        ...
        try:
            ...
            try:
                ...
                if self.observability:
                    ...
                    # Fire-and-forget, but keep references to avoid dangling tasks.
    ...
    def _filter_relevant_results(
        ...
        for result in results:
            ...
            if (
                ...
                and mentioned_year
                and result_year
                and abs(result_year - mentioned_year) <= 1
                and not movies
                and not people
            ):
                # Allow +/- 1 year tolerance for award-year queries.
                is_relevant = True
    ...
    async def close(self) -> None:
        ...
        # Ensure background persistence tasks are cleaned up.
