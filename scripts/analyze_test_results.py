...
def analyze_test_history(db: TestResultsDB, test_name: str, limit: int = 20):
    ...
    for h in history[:limit]:
        timestamp = h.get('timestamp', '')[:19]  # Truncate to date/time
...
def find_flaky_tests(db: TestResultsDB, min_runs: int = 5):
    ...
    for test in flaky_tests:
        print(f"{test['test_name']:<40} {test['total_runs']:<8} "
              f"{test['passed']:<8} {test['failed']:<8} {test['pass_rate']:<12.1%}")
...
def main():
    ...
    parser.add_argument(
        ...
        help='Test results database path (default: test_results.db)'
    )
    parser.add_argument(
        '--pass-rates',
        action='store_true',
        help='Show pass rate analysis'
    )
    parser.add_argument(
        '--test',
        help='Show history for specific test name'
    )
    parser.add_argument(
        '--flaky',
        action='store_true',
    ...
    parser.add_argument(
        '--compare-versions',
        nargs='+',
        help='Compare specific prompt versions'
    )
    parser.add_argument(
        '--days',
        type=int,
    ...
    try:
        ...
        # Default: show pass rates if no specific action
