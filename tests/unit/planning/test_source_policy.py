...
from cinemind.source_policy import SourcePolicy, SourceConstraints, SourceTier, SourceMetadata
...
class TestRequireTierA:
    ...
    def test_require_tier_a_with_only_tier_a_evidence(self, policy):
        ...
        assert all(s.tier == SourceTier.TIER_A for s in filtered_sources), \
            "All sources should be Tier A"
        assert metadata["missing_required_tier"] is False, \
            "Should not flag missing required tier"
...
class TestRejectTierC:
    ...
    def test_reject_tier_c_false_allows_tier_c(self, policy):
        ...
        assert tier_c_count > 0, \
            f"Should have Tier C sources when reject_tier_c=False, got: {tier_c_count}"
...
class TestMetadataAndReasons:
    ...
    def test_metadata_tracks_tiers_present(self, policy):
        ...
        assert metadata["tiers_present_in_candidates"]["A"] > 0, \
            "Should have Tier A in candidates"
        assert metadata["tiers_present_in_candidates"]["B"] > 0, \
    ...
    def test_metadata_tracks_tiers_used(self, policy):
        ...
        assert metadata["tiers_used_in_evidence"]["A"] == \
            sum(1 for s in filtered_sources if s.tier == SourceTier.TIER_A), \
