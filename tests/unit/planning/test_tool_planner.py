...
from cinemind.planning.tool_plan import ToolPlan, ToolPlanner
...
class TestDirectorInfoFreshness:
    ...
    def test_director_info_no_freshness_needed(self, planner):
        ...
        need_freshness, ttl_hours, reason = planner.determine_freshness(
            intent=intent, freshness_signal=freshness_signal, entities=entities, candidate_year=1999
        )

        assert need_freshness is False, f"director_info should not need freshness, got: {need_freshness}"
        assert ttl_hours == 720.0, f"director_info should have long TTL (720h), got: {ttl_hours}"
        assert reason is not None and len(reason) > 0, f"Reason should be set, got: {reason}"
    ...
    def test_director_info_skip_tavily_by_default(self, planner):
        ...
        assert tool_plan.use_imdb_lookup is True, "Should use IMDb lookup for structured sources"
        assert tool_plan.use_cache is True, "Should use cache for stable metadata"
        assert tool_plan.use_structured_db is True, "Should use structured DB for stable metadata"
    ...
    def test_director_info_reason_consistency(self, planner):
        ...
        assert tool_plan.freshness_reason is not None, "freshness_reason should be set"
        assert tool_plan.skip_reason is not None, "skip_reason should be set"
        assert "stable" in tool_plan.freshness_reason.lower() or "metadata" in tool_plan.freshness_reason.lower(), (
            f"freshness_reason should mention stable metadata, got: {tool_plan.freshness_reason}"
        )
        assert (
            "skip" in tool_plan.skip_reason.lower()
            or "cache" in tool_plan.skip_reason.lower()
            or "structured" in tool_plan.skip_reason.lower()
...
class TestWhereToWatchFreshness:
    ...
    def test_where_to_watch_needs_freshness(self, planner):
        ...
        need_freshness, ttl_hours, reason = planner.determine_freshness(
            intent=intent, freshness_signal=freshness_signal, entities=entities
        )

        assert need_freshness is True, f"where_to_watch should need freshness, got: {need_freshness}"
        assert ttl_hours == 6.0, f"where_to_watch should have short TTL (6h), got: {ttl_hours}"
        assert reason is not None and len(reason) > 0, f"Reason should be set, got: {reason}"
    ...
    def test_where_to_watch_does_not_skip_tavily(self, planner):
        ...
        assert tool_plan.use_tavily is True, f"where_to_watch should allow Tavily, got: {tool_plan.use_tavily}"
        ...
        assert tool_plan.use_structured_db is False, "Should skip structured DB for volatile data"
        assert tool_plan.use_cache is True, "Should still check cache (may be stale)"
    ...
    def test_where_to_watch_reason_consistency(self, planner):
        ...
        assert tool_plan.freshness_reason is not None, "freshness_reason should be set"
        assert "volatile" in tool_plan.freshness_reason.lower() or "fresh" in tool_plan.freshness_reason.lower(), (
            f"freshness_reason should mention volatile/fresh, got: {tool_plan.freshness_reason}"
        )
        assert tool_plan.skip_reason is None, "skip_reason should be None when Tavily is allowed"
...
class TestDisambiguationBrowsing:
    ...
    def test_disambiguation_reason_consistency(self, planner):
        ...
        assert tool_plan.freshness_reason is not None, "freshness_reason should be set"
        assert tool_plan.skip_reason is not None, "skip_reason should be set for disambiguation"
...
class TestFreshnessDetermination:
    ...
    def test_stable_intent_with_old_movie(self, planner):
        ...
        assert need_freshness is False, f"Old movie with stable intent should not need freshness, got: {need_freshness}"
        assert ttl_hours == 720.0, f"Should have long TTL for old movie, got: {ttl_hours}"
    ...
    def test_stable_intent_with_freshness_signal(self, planner):
        ...
        assert ttl_hours == 6.0, f"Should have short TTL when freshness signal is True, got: {ttl_hours}"
    ...
    def test_release_date_recent_movie(self, planner):
        ...
        need_freshness, ttl_hours, _reason = planner.determine_freshness(
            intent=intent,
            freshness_signal=freshness_signal,
            entities=entities,
        ...
        assert need_freshness is True, f"Release date for recent movie should need freshness, got: {need_freshness}"
        assert ttl_hours == 6.0, f"Should have short TTL for recent release date, got: {ttl_hours}"
    ...
    def test_release_date_old_movie(self, planner):
        ...
        assert need_freshness is False, f"Release date for old movie should not need freshness, got: {need_freshness}"
        assert ttl_hours == 720.0, f"Should have long TTL for old release date, got: {ttl_hours}"
...
class TestShouldSkipTavily:
    ...
    def test_skip_tavily_with_cache_hit_and_no_freshness(self, planner):
        ...
        should_skip, reason = planner.should_skip_tavily(tool_plan=tool_plan, cache_hit=True, need_freshness=False)

        assert should_skip is True, f"Should skip Tavily with cache hit and no freshness, got: {should_skip}"
        assert "cache" in reason.lower(), f"Reason should mention cache, got: {reason}"

    def test_allow_tavily_with_freshness_needed(self, planner):
        """Test that Tavily is allowed when freshness is needed."""
        tool_plan = ToolPlan(
            use_tavily=True, use_imdb_lookup=True, freshness_reason="volatile intent", skip_reason=None
        )

        should_skip, _reason = planner.should_skip_tavily(tool_plan=tool_plan, cache_hit=False, need_freshness=True)

        assert should_skip is False, f"Should allow Tavily when freshness needed, got: {should_skip}"
        assert tool_plan.tool_plan_skip_tavily is False, "tool_plan_skip_tavily should be False"
    ...
    def test_skip_tavily_follows_tool_plan(self, planner):
        ...
        should_skip, reason = planner.should_skip_tavily(tool_plan=tool_plan, cache_hit=False, need_freshness=False)

        assert should_skip is True, f"Should follow tool plan (skip Tavily), got: {should_skip}"
...
class TestReasonConsistency:
    ...
    def test_freshness_reason_passed_to_tool_plan(self, planner):
        ...
        need_freshness, _ttl_hours, freshness_reason = planner.determine_freshness(
            intent=intent, freshness_signal=freshness_signal, entities=entities, candidate_year=1999
        )
    ...
    def test_skip_reason_explains_tavily_decision(self, planner):
        ...
        assert tool_plan.skip_reason is not None, "skip_reason should be set when Tavily is skipped"
        assert tool_plan.use_tavily is False, "Tavily should be skipped"
        ...
        assert tool_plan2.skip_reason is None, "skip_reason should be None when Tavily is allowed"
        assert tool_plan2.use_tavily is True, "Tavily should be allowed"
...
class TestToolPlanEdgeCases:
    ...
    def test_volatile_intent_always_needs_freshness(self, planner):
        ...
        for intent in volatile_intents:
            ...
            assert ttl_hours == 6.0, f"Volatile intent should have short TTL, got: {ttl_hours}"
            assert "volatile" in reason.lower(), f"Reason should mention volatile, got: {reason}"
    ...
    def test_stable_intent_no_movie_year(self, planner):
        ...
        assert need_freshness is True, f"Should respect freshness signal when no movie year, got: {need_freshness}"
        assert ttl_hours == 6.0, f"Should have short TTL when freshness signal is True, got: {ttl_hours}"
    ...
    def test_tool_plan_structured_db_for_stable(self, planner):
        ...
        assert tool_plan.use_structured_db is True, "Stable intent should use structured DB"
    ...
    def test_tool_plan_no_structured_db_for_volatile(self, planner):
        ...
        assert tool_plan.use_structured_db is False, "Volatile intent should skip structured DB"
